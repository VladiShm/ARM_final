    ...
    public partial class DeleteClients : Form
    {
        ...
        private void LoadInfo()
        {
            try
            {
                ...
                string command2 = "select id, name, surname from client";
        ...
        private void buttonDel_Click(object sender, EventArgs e)
        {
            ...
            {
                sqlCommands.Connection();
                string command2 = "delete from client where id = @id";
                using (var cmd = new NpgsqlCommand(command2, sqlCommands.strCon))
                {
                    cmd.Parameters.AddWithValue("id", idClient[comboBoxClient.SelectedIndex]);
                    cmd.ExecuteNonQuery();
                }
            }
            catch
